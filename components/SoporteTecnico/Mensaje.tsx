...
const Mensaje = ({ contenido, date, id_mensaje, id_ticket, id_usuari } : MensajeTicket) => {
  ...
  return (
    <div style={id_usuari == user.id ? { marginLeft: '25%', borderRadius: '0.5em', backgroundColor: '#0D38A3' } : { marginRight: '25%', borderRadius: '0.5em', backgroundColor: '#3E70EF' }}>
      <Group style={{ justifyContent: 'space-between', paddingLeft: '1em', paddingTop: '0.5em', paddingBottom: '0.5em', paddingRight: '1em' }}>
        <Text style={{ color: 'white', fontWeight: '900' }}>{id_usuari == user.id ? 'Tú' : `Cliente id. ${id_usuari}`}</Text>
